...
st.caption("Gemini 2.5 Flash 搭載。モードを切り替えて効率よく学ぼう！")

# --- 2. 会話履歴の保存場所 ---
if "messages" not in st.session_state:
    st.session_state.messages = []

# ★追加機能：モードが変わった時に履歴をリセットする関数
...
with st.sidebar:
    ...
    # ★★★ モード選択（修正ポイント） ★★★
    # on_change=reset_conversation を追加することで、
    # モードを変更した瞬間に履歴を空っぽにします。
    mode = st.radio(
        "学習モードを選択",
        ["📖 学習モード", "⚡ 解答確認モード", "⚔️ 演習モード"],
        index=0,
        on_change=reset_conversation  # ← これがバグ修正の鍵です
    )
