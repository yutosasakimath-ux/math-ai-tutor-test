...
with st.sidebar:
    ...
    # --- ■ 3. 演習モードの機能 ---
        ...
        # ★以前あった「採点スタイルの選択」を削除し、シンプルにしました。
...
if mode == "📖 学習モード":
    ...
    # ★ここを修正：スタイル選択を廃止し、常に「詳細解説」を行うよう統一
    system_instruction = base_instruction + """
    【役割：試験監督・コーチ】
    ...
    1. **正解の場合**: 
       - 「正解です！」と褒めてください。
       - **必ず、正しい解法プロセスとポイントを詳しく解説してください**（解答解説セット）。
    2. **不正解の場合**: 
       - **絶対にまだ「正しい答え」を教えないでください**。
       - 「不正解です」とだけ伝え、どこに着目すべきか**ヒント**を出してください。
       - 「もう一度解いてみますか？それとも解答を見ますか？」と生徒に問いかけてください。
    3. **ギブアップの場合**: 
       - 生徒が「ギブアップ」「答えを教えて」と言った場合のみ、正解と詳しい解説を提示してください。
    4. **類題作成の場合**: 
       - 直前の問題の数値を変えた、同じ難易度の問題を出題してください。
    """
